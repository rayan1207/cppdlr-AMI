...
void ggm_mDLR::graphlist_to_mDLRlist(){
	ord_list.resize(graphlist.size());
...
Bz_container ggm_mDLR::generate_SE_local(mDLR &_mDLR, nda::array<dcomplex,2> &fk  ){                  
	auto momenta_kernel = _mDLR.compute_local_momenta_kernel();
	if (rank ==0) {std::cout << "Momenta kernel computed \n";}
    return _mDLR.MPI_vdot_freq_momenta_kernel_local(momenta_kernel, fk);
}




Bz_container ggm_mDLR::generate_summed_SE(int iter,std::string type, bool write, int max_ord){
	std::vector<Bz_container> SE_list(graph_size);
	if (type == "nonlocal") {
	for (int i =0; i< graph_size; i++){
        if (ord_list[i] <= max_ord){
		auto SE =  generate_SE( mDLR_list[i], Fk_ggm[i]);
		mDLR_list[0].denoise_FS_points(SE);
		if (write){
		auto name = graphlist_names[i];
		std::string file_SE ="data/Individual_data/SE_nlocal_" + name + "_i" + std::to_string(i) + ".txt";
	...
	for (int i =0; i< graph_size; i++){
		if (ord_list[i] <= max_ord){
		auto SE =  generate_SE_local( mDLR_list[i], Fk_ggm[i]);
			if (write){
			auto name = graphlist_names[i];
			std::string file_SE ="data/Individual_data/SE_local_" + name + "_i" + std::to_string(i) + ".txt";
			mDLR_list[i].write_data_momenta(file_SE,SE, master_mfreq);	
			}
		SE_list[i] = std::move(SE);
		}
	...
	std::cout << " Choose either local or nonlocal \n";
}
...
void ggm_mDLR::initialize_ggm_DLR_W(){
...
void ggm_mDLR::initialize_ggm_DLR_W(Bz_container &GF){
	for (auto &mDLR : mDLR_list){
...
void ggm_mDLR::transfer_local_ggm_DLR_W(nda::array<dcomplex,1>& G_loc){
	for (auto &mDLR : mDLR_list){
		mDLR.populate_local_master_dlrW(G_loc);
		mDLR.transfer_local_master_DLR_weights_to_dlrR0_elements();
	}	
}

void ggm_mDLR::initialize_local_ggm_DLR_W(nda::array<dcomplex,1>& G_loc){
	for (auto &mDLR : mDLR_list){
		mDLR.populate_local_master_dlrW(G_loc);
		mDLR.transfer_local_master_DLR_weights_to_dlrR0_elements();
	}	
...
void ggm_mDLR::initialize_local_ggm_DLR_W(){
	for (auto &mDLR : mDLR_list){
		mDLR.populate_local_master_dlrW();
		mDLR.transfer_local_master_DLR_weights_to_dlrR0_elements();
	}	
}
...
void ggm_mDLR::ScPT_DMFT_solver(int max_iters, Bz_container &final_SE, Bz_container &final_GF,bool write){
	...
	auto SE_new = generate_summed_SE(iter_idx,type,write,params.ord_max);
	...
	if (std::abs(mu) <TOLERANCE & params.tp ==0){
	mDLR_list[0].set_re_zero(SE);
	...
	double mu_new = (std::abs(params.target_n - density) < TOLERANCE)
						? mu
	...
	Bz_container GF_weiss_nl;
	nda::array<dcomplex,1> GF_weiss_l;
	GF= mDLR_list[0].G_from_DLR_SE_M(SE, master_mfreq, mu_new);
	GF_weiss_l = 1/(1/average_Bz_container(GF) +SE[0][0]);

    transfer_local_ggm_DLR_W(GF_weiss_l);
	if (write){
...
void ggm_mDLR::ScPT_LSEET_solver(int max_iters, Bz_container &final_SE, Bz_container &final_GF,bool write){
	...
	auto SE_K = generate_summed_SE(iter_idx,"nonlocal",write,2);
	auto SE_local = generate_summed_SE(iter_idx,"local",write,params.ord_max);
	...
	auto SE_new = mDLR_list[0].embed_localSE_to_SE_K( SE_K,SE_local); // SE_k +SE_local - \sum(SE_k)

	SE = ( iter_idx < 1) ? SE_new : mDLR_list[0].SE_mixer(SE_old,SE_new, alpha);

	if (params.target_n ==1 && params.tp ==0){
	std:: cout << "Denoising data FS points: \n";
	mDLR_list[0].denoise_FS_points(SE);
	mDLR_list[0].symmetrize_fermionic_DLR_Bz(SE);
	mDLR_list[0].denoise_FS_points(SE);
	}
	...
	Bz_container GF_weiss;
	 ...
	 if (params.DCA==1){
		...
		GF_weiss = mDLR_list[0].generate_weiss_G(master_mfreq,SE, GF_bar);
		transfer_ggm_DLR_W(GF_weiss);
		auto GF_weiss_local =average_Bz_container(GF_weiss);
		transfer_local_ggm_DLR_W( GF_weiss_local);
		
	}

	else {
		transfer_ggm_DLR_W(GF);
		auto GF_local =average_Bz_container(GF);
		transfer_local_ggm_DLR_W( GF_local);
	}
